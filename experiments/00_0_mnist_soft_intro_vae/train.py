...
def train():
    ...
    # dataset = DatasetMNIST(img_size=28, batch_size=BATCH_SIZE, num_workers=4, normalization="none")
    dataset = DatasetMNISTBackground(img_size=28, batch_size=BATCH_SIZE, num_workers=4, normalization="none")
    dataset.preload_dataset(train=True)

    vae_model = CustomSoftIntroVAE(cdim=1, zdim=latent_size, channels=(64, 128), image_size=28, conditional=False)

    optimizer_e = torch.optim.Adam(vae_model.encoder.parameters(), lr=2e-5)
    optimizer_d = torch.optim.Adam(vae_model.decoder.parameters(), lr=2e-5)
